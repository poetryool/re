...
parser.add_argument('--cuda', action='store_true',
                    help='use CUDA')
parser.add_argument('--log_interval', type=int, default=200, #指定日志输出间隔
                    help='report interval')
parser.add_argument('--checkpoint', type=str, default='./pepler/', #指定保存模型的目录路径
                    help='directory to save the final model')
parser.add_argument('--outf', type=str, default='generated.txt',
                    help='output file for generated text')
parser.add_argument('--endure_times', type=int, default=5,
                    help='the maximum endure times of loss increasing on validation')
parser.add_argument('--rating_reg', type=float, default=0.01, #推荐任务的正则化权重。
                    help='regularization on recommendation task')
parser.add_argument('--text_reg', type=float, default=1.0, #文本生成任务的正则化权重。
                    help='regularization on text generation task')
parser.add_argument('--use_mf', action='store_true', #是否使用矩阵分解（Matrix Factorization）作为模型，否则使用 MLP 模型
                    help='otherwise MLP')
...
#如果目录不存在，用os.makedirs()创建该目录，通过os.path.join()构建模型路径和预测路径，用于保存模型和预测结果
...
feature_set = corpus.feature_set #数据加载器中的特征集合
...
nuser = len(corpus.user_dict) #获取用户字典的长度，表示用户的数量
nitem = len(corpus.item_dict) #获取物品字典的长度，表示物品的数量
ntoken = len(tokenizer) #获取分词器的词汇表大小，表示模型的输入词汇数量
model = RecReg.from_pretrained(r'/root/autodl-tmp/poetry/llm/gpt2', nuser, nitem, args.use_mf, local_files_only=True)
model.resize_token_embeddings(ntoken)  # 调整模型的嵌入层大小，使其适应输入词汇的数量three tokens added, update embedding table
model.to(device)
rating_criterion = nn.MSELoss() # 定义MSE作为评价指标，用于度量评分预测的误差
...
def train(data):
    ...
    model.train() #将模型设置为训练模式（启用 dropout）
    text_loss = 0. #初始化文本损失和评分损失为零
    rating_loss = 0.
...
# Loop over epochs.
best_val_loss = float('inf') #跟踪最佳的验证损失和
endure_count = 0 #验证损失没有改善的次数
for epoch in range(1, args.epochs + 1):
    ...
    #当前的验证损失小于之前记录的最佳验证损失，更新最佳验证损失，并将模型保存到指定的文件中
